        ...
        export function truncateAxis(text, width, textProperties?: TextProperties) {
             text.each(function() {
                var text = d3.select(this),
                    truncatedText = TextUtility.getTailoredTextOrDefault({text: text.text(), fontFamily: (textProperties ? textProperties.fontFamily : 'sans-serif'), fontSize: (textProperties ? textProperties.fontSize  : '11px')}, width);
